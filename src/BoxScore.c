...
//pull from FileIO not dig...
  ...
  folder = "setting/";/// +  expName;
  file = new FileIO(rootFileName);
...
int PlaneSetting(string location){
  if( location == "testing") {
    gp = new GenericPlane();
  ...
  if( c == 'c' && dig->GetAcqMode() == "list"){ ////========== pause and make cuts, only for list mode
    ...
    if(opt==1){
       ...
       if(cutcheck != nullptr){
          if(cutcheck->IsOpen()){
             cutcheck->Close();
             archiveCutFile.Form("data/ArchiveCut_%s", rootFileName.Data());
