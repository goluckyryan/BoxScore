...
#define MAXNPORT 4
#define MAXNB 8 /* Number of connected boards per port */
...
int main(int argc, char* argv[]){
  ...
  for(p = 0; p < MAXNPORT; p ++){
    for(b = 0; b < MAXNB; b++){
      ...
      printf("========================= boardID = %d / %d \n", b, MAXNB-1);
      ret = CAEN_DGTZ_OpenDigitizer(CAEN_DGTZ_USB, b,0,0,&handle[p][b]);
        
      if ( ret != CAEN_DGTZ_Success) {
        printf("          USB : can't open digitizer. probably not connected.\n");
        ret = CAEN_DGTZ_OpenDigitizer(CAEN_DGTZ_OpticalLink, p , b, 0, &handle[p][b]);
        if(ret != CAEN_DGTZ_Success) {
      ...
      printf("\tNumber of channels : \e[31m%d \e[0m\n", BoardInfo.Channels);
      ...
      int numProbes;
      ret = CAEN_DGTZ_GetDPP_SupportedVirtualProbes(handle[p][b], 1, probes, &numProbes);
...
QuitProgram:
    ...
    for(p=0; p<MAXNPORT; p++){
      for(b=0; b<MAXNB; b++)   ret = CAEN_DGTZ_CloseDigitizer(handle[p][b]);
    }
    printf("===========================\n");
